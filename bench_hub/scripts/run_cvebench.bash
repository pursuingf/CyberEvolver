...
SEED_INCLUDE="${SEED_INCLUDE:-commands/check_done.py}"
...
CTF_NAMESPACE="${CTF_NAMESPACE:-${RUN_ID_NAMESPACE_PART}_${MODEL_NAMESPACE_PART}}"
...
banner "Stage 1/8: cy_agent baseline zero_day"
run_cmd "${PYTHON_BIN}" "${BASELINE_SCRIPT}" \
  "${BASELINE_COMMON_ARGS[@]}" \
  --run-id "${RUN_ID_PREFIX}_baseline_zero_day" \
  --prompt-variant zero_day

banner "Stage 2/8: t_agent baseline zero_day"
run_cmd "${PYTHON_BIN}" "${BASELINE_SCRIPT}" \
  "${BASELINE_T_AGENT_ARGS[@]}" \
  --run-id "${RUN_ID_PREFIX}_t_agent_zero_day" \
  --prompt-variant zero_day

banner "Stage 3/8: cy_agent baseline one_day"
run_cmd "${PYTHON_BIN}" "${BASELINE_SCRIPT}" \
  "${BASELINE_COMMON_ARGS[@]}" \
  --run-id "${RUN_ID_PREFIX}_baseline_one_day" \
  --prompt-variant one_day

banner "Stage 4/8: t_agent baseline one_day"
run_cmd "${PYTHON_BIN}" "${BASELINE_SCRIPT}" \
  "${BASELINE_T_AGENT_ARGS[@]}" \
  --run-id "${RUN_ID_PREFIX}_t_agent_one_day" \
  --prompt-variant one_day

banner "Stage 5/8: raw zero_day"
run_cmd "${PYTHON_BIN}" "${EVOLVE_SCRIPT}" \
  "${EVOLVE_COMMON_ARGS[@]}" \
  --config-mode raw \
  --run-id "${RUN_ID_PREFIX}_raw_zero_day" \
  --prompt-variant zero_day

banner "Stage 6/8: evo zero_day"
run_cmd "${PYTHON_BIN}" "${EVOLVE_SCRIPT}" \
  "${EVOLVE_COMMON_ARGS[@]}" \
  --config-mode evo \
  --run-id "${RUN_ID_PREFIX}_evo_zero_day" \
  --prompt-variant zero_day

banner "Stage 7/8: raw one_day"
run_cmd "${PYTHON_BIN}" "${EVOLVE_SCRIPT}" \
  "${EVOLVE_COMMON_ARGS[@]}" \
  --config-mode raw \
  --run-id "${RUN_ID_PREFIX}_raw_one_day" \
  --prompt-variant one_day

banner "Stage 8/8: evo one_day"
run_cmd "${PYTHON_BIN}" "${EVOLVE_SCRIPT}" \
  "${EVOLVE_COMMON_ARGS[@]}" \
  --config-mode evo \
  --run-id "${RUN_ID_PREFIX}_evo_one_day" \
  --prompt-variant one_day
