...
from llm import create_client, track_usage
from tools import load_all_tools

CLAUDE_MODEL = "anthropic/claude-sonnet-4"
OPENAI_MODEL = "gpt-5"

def _auto_check_scoring(tool_result="", logging=print):
    """Check if challenge is solved after each tool call.

    Mechanisms (no plaintext flag is ever read here):
      1. http_poll:     poll SCORING_URL endpoint (CVEBench)
      2. flag verifier: if FLAG_VERIFY_URL is set and the tool just emitted a
                        SUBMISSION sentinel, POST the submitted flag to the
                        host-side validator and trust its response.

    Returns True when the challenge is solved, False otherwise.
    """
    import os, json, re, urllib.request

    # 1. http_poll: check scoring endpoint (CVEBench)
    url = os.environ.get("SCORING_URL", "")
    if url:
        try:
            ...
            with urllib.request.urlopen(req, timeout=10) as resp:
                ...
                if data.get("status"):
                    logging("[auto-check] scoring endpoint returned solved=true")
                    return True
        except Exception:
            pass

    # 2. flag verifier: if a SUBMISSION sentinel just appeared, ask the host.
    verify_url = os.environ.get("FLAG_VERIFY_URL", "").strip()
    if verify_url and tool_result:
        matches = re.findall(r"<<SUBMISSION\|\|(.*?)\|\|SUBMISSION>>", str(tool_result))
        if matches:
            submitted = matches[-1].strip()
            try:
                req = urllib.request.Request(
                    verify_url,
                    data=json.dumps({"flag": submitted}).encode("utf-8"),
                    method="POST",
                ...
                if payload.get("correct"):
                    logging("[auto-check] verifier confirmed correct flag")
                    return True
            except Exception:
                pass
...
def chat_with_agent_openai(
    ...
    try:
        ...
        for i in range(max_llm_calls):
            if timeout * 0.9 < time() - start_time:
                ...
                return new_msg_history, i, solved
            ...
            if not tool_use:
                return new_msg_history, i + 1, solved
            # Process tool call
            ...
            # Harness-level auto-check: poll scoring endpoint after every step
            if _auto_check_scoring(tool_result=tool_result, logging=logging):
                solved = True
                return new_msg_history, i + 1, solved
    ...
    return new_msg_history, max_llm_calls, solved
...
def chat_with_agent_open_router(
    ...
    try:
        ...
        for i in range(max_llm_calls):
            ...
            # Harness-level auto-check: poll scoring endpoint after every step
            if _auto_check_scoring(logging=logging):
                solved = True
                return new_msg_history, i + 1, solved
...
def chat_with_agent(
    ...
    if model.startswith("o") or "gpt" in model.lower():
        # OpenAI models
        new_msg_history, n_llm_calls, solved = chat_with_agent_openai(
            msg,
            model=model,
        ...
        new_msg_history, n_llm_calls, solved = chat_with_agent_open_router(
            msg,
            model=model,
            msg_history=msg_history,
            logging=logging,
    ...
    return new_msg_history, n_llm_calls, solved
...
if __name__ == "__main__":
    ...
    history, _, _ = chat_with_agent(msg, model=model, max_llm_calls=2)
